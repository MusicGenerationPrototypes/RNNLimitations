...
from musicsource.musicdata import Batch
...
class Model:
            ...
            if policy == Model.ScheduledSamplingPolicy.NONE:
                ...
                def linear_policy(step):
                    if step < first_step:
                        ...
                        slope = (init_value-end_value)/(first_step-last_step)  # < 0 (because last_step>first_step and init_value>end_value)
    ...
    def __init__(self, args):
        ...
        self.current_lr = None  # Allow to have a dynamic learning rate
    ...
    def _build_network(self):
        ...
        with tf.name_scope('placeholder_use_prev'):
            self.use_prev = [
                tf.placeholder(
                    ...
                    [],
                    name='use_prev')
                for _ in range(self.args.sample_len)  # The first value will never be used (always takes self.input for the first step)
                ]
        ...
        (outputs, self.final_state) = tf.nn.seq2seq.rnn_dec(
        ...
        if not self.args.test:
            ...
            self.learning_rp = Model.LearningRatePolicy(self.args)  # Load the chosen policies


            loss_fct = tf.nn.seq2seq.sequence_loss(
                self.outputs,
                self.targets,
                [tf.constant(self.target_wp.get_weight(i), shape=self.targets[0].get_shape()) for i in range(len(self.targets))],  # Weights
                softmax_loss_function=tf.nn.sigmoid_cross_entropy_with_logits,
                average_across_timesteps=False,  # I think it's best for variables length sequences (specially with the target weights=0), isn't it (it implies also that short sequences are less penalized than long ones)
    ...
    def step(self, batch, train_set=True, glob_step=-1, ret_output=False):
        ...
        if not self.args.test:  # Training
            ...
            for i in range(self.args.sample_len):
                ...
                #if not train_set or np.random.rand() > self.schedule_plc.get_prev_threshold(glob_step)*self.target_wp.get_weight(i):  # Regular Schedule sample
                if np.random.rand() > self.schedule_plc.get_prev_threshold(glob_step):  # Weight the threshold by the target weights (don't schedule sample if weight=0)
