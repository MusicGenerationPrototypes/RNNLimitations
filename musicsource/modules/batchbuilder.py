"""
Бач-билдер, переводящий песни в массивы данных, понятных нс.
...
class Relative(BatchBuilder):
    ...
    def process_song(self, old_song):
        ...
        for note in all_notes[1:]:
            ...
            new_note.scale = (note.note//Relative.NB_NOTES_SCALE - prev_note.note//Relative.NB_NOTES_SCALE) % Relative.NB_SCALES
    ...
    def reconstruct_batch(self, output, batch_id, chosen_labels=None):
        ...
        processed_song.first_note.note = 56
        print('Reconstruct')
    ...
    def get_list(self,  dataset, name):
        ...
        extracts = []
        sample_subsampling_length = self.args.sample_len+1
        for song in dataset:
            len_song = len(song.notes)
            max_start = len_song - sample_subsampling_length
            assert max_start >= 0  
            nb_sample_song = 2*len_song // self.args.sample_len
            for _ in range(nb_sample_song):
                extracts.append(self.create_extract(
                    song,
                    random.randrange(max_start),
                    self.args.sample_len
                ))
...
class PianoRoll(BatchBuilder):
    ...
    def get_list(self, dataset):
        ...
        sample_subsampling_length = self.args.sample_len+1  # We add 1 because each input has to predict the next output
        ...
        for song in songs_set:
            ...
            max_start = len_song - sample_subsampling_length
            assert max_start >= 0
            nb_sample_song = 2*len_song // self.args.sample_len  # The number of subsample is proportional to the song length
            for _ in range(nb_sample_song):
